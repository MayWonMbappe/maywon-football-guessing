...
const FOOTBALL_EXTRA_QUESTIONS = [
  ...
  { id: "age_29_plus", text: "TA 现在已经 29 岁或更大吗？", chip: "29岁以上", weight: 0.98, family: "age" },
...
function applyExternalFootballData() {
  ...
  const candidates = window.FOOTBALL_PLAYERS.map((entry) => ({
    id: entry.id,
    name: entry.name,
    title: entry.title,
    bio: entry.bio,
    ...
    aliases: new Set([
      ...(entry.aliases || []),
      ...(window.FOOTBALL_ZH_ALIASES && window.FOOTBALL_ZH_ALIASES[entry.name]
        ? [window.FOOTBALL_ZH_ALIASES[entry.name]]
        : []),
    ]),
    meta: entry.meta || {},
  }));
...
function polishFootballQuestion(entry) {
  ...
  const [text, chip] = overrides[entry.id] || [entry.text, entry.chip];
  return {
    id: entry.id,
    text,
    chip,
    ...
    family: entry.family || questionFamilyFromId(entry.id),
  };
}
...
function enrichFootballCandidate(candidate) {
  ...
  if (age !== null) {
    ...
    if (age >= 29) traits.add("age_29_plus");
  }
...
function dynamicQuestionFromId(id) {
  ...
  if (id.startsWith("name_has_")) {
    ...
    return {
      id,
      text: `TA 的英文名字里含有字母 ${letter.toUpperCase()} 吗？`,
...
function expectedForQuestion(candidate, q) {
  ...
  if (q.family === "club" && candidate.meta && !candidate.meta.club) return null;
  ...
  if (q.id.startsWith("name_has_")) {
    return normalizeText(candidate.name).includes(q.id.replace("name_has_", "")) ? 1 : 0;
  }
...
function chooseQuestion(scores) {
  ...
  const availableQuestions = [...dataset.questions, ...dynamicQuestions(scores)];
  ...
  const targeted = targetedQuestionAfterFifteen(scores, questionPool);
  if (targeted) return targeted;

  const poolSize = dataset === DATASETS.football ? 24 : 10;
  const pool = scores.slice(0, Math.min(poolSize, scores.length));
...
function dynamicQuestions(scores) {
  if (dataset !== DATASETS.football || state.answers.length < 14 || !scores.length) return [];
  const leaders = scores
    .filter((item) => (item.matchRate || 0) >= Math.max(0.52, (scores[0].matchRate || 0) - 0.14))
    .slice(0, 8)
...
function dynamicLetterQuestions(candidates) {
  ...
  return letters
    .map((letter) => {
      const count = candidates.filter((candidate) => normalizeText(candidate.name).includes(letter)).length;
    ...
    .map((item) => ({
      ...
      text: `TA 的英文名字里含有字母 ${item.letter.toUpperCase()} 吗？`,
...
function dynamicTeammateQuestions(candidates) {
  const byClub = new Map();
  ...
  for (const [club, group] of byClub.entries()) {
    ...
    const teammate = notableTeammateForClub(club, group);
    if (!teammate) continue;
    questions.push({
      id: `teammate_${slugTrait(club)}_${slugTrait(teammate.name)}`,
      text: `TA 现在和 ${displayName(teammate)} 是俱乐部队友吗？`,
...
function notableTeammateForClub(club, group) {
  const pool = dataset.candidates.filter((candidate) => candidate.meta && candidate.meta.club === club);
  const outsideGroup = pool.filter((candidate) => !group.some((item) => item.id === candidate.id));
  return outsideGroup.find((candidate) => /Tonali|Saka|Rice|Haaland|Bowen|Paqueta|Paquetá|Isak|Guimarães|Wirtz|Schick/i.test(candidate.name))
    || outsideGroup.sort((a, b) => Number(b.meta.appearanceShare || 0) - Number(a.meta.appearanceShare || 0))[0]
    || group[0];
}
...
function phaseMultiplier(q) {
  ...
  if (n < 8) return ["country", "scope", "position", "region"].includes(family) ? 1.35 : lateDetail ? 0.04 : family === "club" ? 0.08 : 0.45;
  if (n < 15) return ["club", "specific_position", "status"].includes(family) ? 1.18 : lateDetail ? 0.12 : family === "position" ? 1.1 : 0.85;
...
function isUsefulClubQuestion(q, scores) {
  ...
  if (state.answers.length < 12) return false;
  const leaders = scores
    .filter((item) => (item.matchRate || 0) >= Math.max(0.5, (scores[0]?.matchRate || 0) - 0.14))
    .slice(0, 7);
  if (leaders.length < 2) return false;
  const yes = leaders.filter((item) => item.candidate.traits.has(q.id));
  ...
  const noisySmallClub = /St\\. Pauli|Burnley|Heidenheim|Damac|Mallorca|West Ham|Wolverhampton/i.test(clubName);
  return broadOrLeaderClub && !noisySmallClub;
}
...
function shouldShowLowSignalNotice(scores) {
  ...
  const top = scores[0];
  if (!top) return false;
  return effectiveAnswerCount() < settings.minEffectiveAnswers || top.matchRate < settings.lowSignalConfidence;
}
...
function render() {
  ...
  if (currentQuestion) {
    ...
    const topRate = latestScores[0] ? latestScores[0].matchRate || 0 : 0;
    const enoughSignal =
      effectiveAnswerCount() >= (dataset.guess.minEffectiveAnswers || 0) &&
      topRate >= (dataset.guess.lowSignalConfidence || 0);
    els.questionText.textContent = enoughSignal
      ? "我已经没有更好的问题了。"
      : "线索还不够明确，请公布答案计算匹配率。";
    if (!dataset.guess.hardGate && enoughSignal) setTimeout(() => showResult(latestScores), 180);
  }
}
...
function answerQuestion(answerKey) {
  ...
  window.setTimeout(() => {
    state.answers.push({ questionId: currentQuestion.id, answerKey });
...
function showLowSignalNotice(matchRate) {
  ...
  layer.innerHTML = `
    <section class="notice-sheet" role="dialog" aria-modal="true" aria-labelledby="noticeTitle">
      ...
      <h2 id="noticeTitle">我还需要更明确的线索</h2>
      <p>已经问到第 ${state.answers.length} 问，但当前最高相关度约 ${Math.round(matchRate * 100)}%，有效回答 ${effective}/${needed}。请尽量减少“不确定”，再给我几个明确回答。</p>
...
els.skipBtn.addEventListener("click", () => answerQuestion("unknown"));
